...
# ╔═╡ 64a96f2c-16d1-11ef-3016-1fc99371a65e
using Plots, Printf, LinearAlgebra, DelimitedFiles, PlutoUI, Roots, Symbolics,OrdinaryDiffEq

# ╔═╡ c285fcb7-8c68-48a5-9746-c9a7aa802314
using HomotopyContinuation#: solve as homotopy_solve, System

# ╔═╡ 2251a2a7-9e12-4849-826a-7bdaca288a25
const ode_solve = OrdinaryDiffEq.solve

# ╔═╡ cf03d51f-1186-4d54-b064-3a5380eefc55
const homotopy_solve = HomotopyContinuation.solve

# ╔═╡ a8d4e3dc-8e66-4e09-93d6-28fa1ad8b479
#const ode_expand = OrdinaryDiffEq.expand

# ╔═╡ 1db95ea2-c065-43de-9ecb-675941ae59ff
PlutoUI.TableOfContents(title="", aside=true)

# ╔═╡ 75a88e5a-fdf4-48ca-8ae0-ad511cd6018d
md"""
# Definitions
"""

# ╔═╡ 600ab04a-dcb2-4071-90b7-07d01dbff9cc
function W_fermi(ΔP, K)
	return 1.0 / (1.0 + exp(-ΔP / K))
end

# ╔═╡ 418c20f9-5809-452d-ac40-bed7cf651bf1
@variables ρc R S T P K ncc ncd 

# ╔═╡ 6b7922b7-349a-4600-b000-024927c4d211
@var ρcc ρcd

# ╔═╡ f018747a-1166-4ade-8aa6-3af5dd49ac43
begin
	k = 4
	R_value = 1.0
	P_value = 0.0
	...
	T_range = 0:0.1:2
	S_range = -1:0.1:1
	# in SH if T_value < 1 && S_value < 0
end

# ╔═╡ bdec861d-d7e9-4b05-abee-0e47ef889056
md"""
# First approach
"""

# ╔═╡ ef0c5e55-b717-42bb-a45d-6850244e09b8
md"""
```math
\Pi_C = \rho_C R + (1 - \rho_C) S
```
"""

# ╔═╡ a934dc60-e864-472c-947b-eb8157497945
md"""
```math
\Pi_D = \rho_C T + (1 - \rho_C) P
```
"""

# ╔═╡ c8387b4f-a17d-4af3-99e1-3a1d84b3cda7
md"""
```math
    \dot{\rho_{C}} = (1 - \rho_{C}) \rho_{C} \frac{1}{1 + e^{-\beta \Delta}}
- \rho_{C}  (1 -\rho_{C}) \frac{1}{1 + e^{+\beta \Delta}}
```
```math
= (1 - \rho_{C}) \rho_{C} \tanh\left(\frac{\beta \Delta}{2}\right) 
```
"""

# ╔═╡ 93c9f8b4-c255-4761-a3d8-daed36506cf9
function payoff_C(ρc, R, S)
	return  ρc*R + (1 - ρc)*S
end

# ╔═╡ 952536b7-fb93-44f5-b66c-7b634a9eca0b
function payoff_D(ρc, T, P)
	return  ρc*T + (1 - ρc)*P
end

# ╔═╡ b52b6e06-8494-4eb3-85f3-eaad21dd8781
md"""
## Master eq.
"""

# ╔═╡ 97a526c0-027e-4124-b1d2-9719257eceba
# Master eq.
function eq_c(ρc, R, S, T, P, K)
	
	Pc = payoff_C(ρc, R, S)  # Compute symbolic payoff for C
	Pd = payoff_D(ρc, T, P)  # Compute symbolic payoff for D		    
	ΔP = Pc - Pd  # Compute ΔP
	
	eq_c = #(1 - ρc) * ρc * tanh(ΔP / (2*K))
		(1 - ρc) * ρc * W_fermi(ΔP, K) - ρc * (1 - ρc) * W_fermi(-ΔP, K)  
	
	return eq_c 
end

# ╔═╡ feafe8db-e053-46e2-adad-1c39b7d330eb
eq_c(ρc, R, S, T, P, K)

# ╔═╡ 71af07e8-1139-45e5-9fc4-f8e540757ea1
md"""
## Solutions
"""

# ╔═╡ 290abd82-cf55-4265-a49d-55bb9aaa78a9
begin

	selected_solutions_matrix = fill(NaN, length(T_range), length(S_range))
	...
	for t in T_range
		...
		for s in S_range
			...
			function eq_c_numeric!(du, u, p, t)
				ρc = u[1]
				du[1] = eq_c(ρc, R_value, S_value, T_value, P_value, K_value)
			end
			...
			# Solve the ODE
			prob = ODEProblem(eq_c_numeric!, u0, tspan)
			solution = ode_solve(prob, Tsit5())
			println("$T_value $S_value $(solution[end][1])")

			selected_solutions_matrix[T_index,S_index] = solution[end][1]
			S_index += 1
		end
		T_index += 1	
	end
...
# ╔═╡ 15d8aeb1-0fe2-4097-bac4-83468dca44e3
md"""
## Phase space
"""

# ╔═╡ 4db42dac-18a9-4a7a-8a86-81150883b497
begin
		# Plot the heatmap of the selected solutions
		heatmap_plot = heatmap(T_range, S_range, selected_solutions_matrix',
        #color=:rainbow,
        clims=(0,1),
        xlabel="T values", ylabel="S values",
        title="Heatmap of Selected Solutions")

		plot(heatmap_plot)
		# Add text annotation at a specific position (T, S)
		annotate!(heatmap_plot, 0.5, 0.5, text("HG", :black, 10))
		annotate!(heatmap_plot, 1.5, 0.5, text("SD", :black, 10))
		annotate!(heatmap_plot, 0.5, -0.5, text("SH", :white, 10))
		annotate!(heatmap_plot, 1.5, -0.5, text("PD", :white, 10))
		plot!(heatmap_plot, [1, 1], [-1, 1], color=:white, linewidth=2, legend=:none)
		plot!(heatmap_plot, [0, 2], [0, 0], color=:white, linewidth=2, legend=:none)

		# Display the plot
		#show(heatmap_plot)
end

# ╔═╡ a898fd4c-794d-4e4d-8a2a-1a8fefe4b91f
md"""
# Second approach
"""

# ╔═╡ 449df668-ec30-4fdc-b1b4-709a98c1c6a3
md"""
```math
\Pi_C = n_{CC} R + (k - n_{CC}) S
```
"""

# ╔═╡ be098e71-2781-4c44-968d-565dee21e594
md"""
```math
\Pi_D = n_{CD} T + (k - n_{CD}) P
```
"""

# ╔═╡ 9d44689c-3ccd-4d47-a9b2-72c2c5455a4a
md"""
```math
\dot{\rho_{C}} = (1 -\rho_{C}) \sum_{n_{CD}=0}^{k} \binom{k}{n_{CD}} \rho_{C}^{n_{CD}} (1 -  \rho_C)^{(k - n_{CD})} 
```
```math
\frac{n_{CD}}{k} \sum_{n_{CC}=0}^{k-1} \binom{k}{n_{CC}} \rho_{C}^{n_{CC}} (1 -  \rho_C)^{(k - n_{CC})} \frac{1}{1 + e^{-\beta \Delta^{i}(n_{CC},n_{CD})}}
```
```math
- \rho_{C}  \sum_{n_{CC}=0}^{k} \binom{k}{n_{CC}} \rho_{C}^{n_{CC}} (1 -  \rho_C)^{(k - n_{CC})} 
```
```math
 \frac{n_{DC}}{k}   \sum_{n_{CD}=1}^{k} \binom{k}{n_{CD}} \rho_{C}^{n_{CD}} (1 -  \rho_C)^{(k - n_{CD})} \frac{1}{1 + e^{+\beta \Delta^{i}(n_{CC},n_{CD})}} 
```
"""

# ╔═╡ 47002afc-c777-4bc7-a87a-1d108b400c66
function payoff_C2(nc, R, S)
	return  nc*R + (k - nc)*S
end

# ╔═╡ 00c3e268-4ec7-4425-b7fa-d7a88b87a7e8
function payoff_D2(nc, T, P)
	return  nc*T + (k - nc)*P
end

# ╔═╡ 9df25952-a2ae-43e7-b05d-1408c877cd0a
md"""
## Master eq.
"""

# ╔═╡ 20a8e12d-1496-4597-a08a-191f7b9491c4
function eq_c2(ρc, R, S, T, P, K)
	
	Pc = payoff_C2(ncc, R, S)  # Compute symbolic payoff for C
	Pd = payoff_D2(ncd, T, P)  # Compute symbolic payoff for D		    
	ΔP = Pc - Pd  # Compute ΔP

	eq_c2 = 
		(1 - ρc) * ρc * sum(binomial(k,ncd) * ρc^ncd * (1 - ρc)^(k - ncd) * binomial(k,ncc) * ρc^ncc * (1 - ρc)^(k - ncc) * W_fermi(payoff_C2(ncc, R, S)-payoff_D2(ncd, T, P), K) for ncd in 0:k for ncc in 0:k-1) - 
		ρc * (1 - ρc) * sum(binomial(k,ncc) * ρc^ncc * (1 - ρc)^(k - ncc) * binomial(k,ncd) * ρc^ncd * (1 - ρc)^(k - ncd) * W_fermi(-payoff_C2(ncc, R, S)+payoff_D2(ncd, T, P), K) for ncc in 0:k for ncd in 1:k)
	
	return eq_c2
end

# ╔═╡ c3201827-c9ea-4bad-aa35-040b7d086d00
eq_c2(ρc, R, S, T, P, K)

# ╔═╡ 2403fe56-c843-4912-95cf-55d30955cf1f
Symbolics.expand(eq_c2(ρc, 1, 0.9, 1.1, 0, 0.1))

# ╔═╡ d714e935-90c5-478c-816d-6facf9fa8cf4
md"""
## Solutions
"""

# ╔═╡ 9e2ce15b-7bf4-4e18-b1b3-89f4378979bc
begin

	selected_solutions_matrix2 = fill(NaN, length(T_range), length(S_range))
	...
	for t in T_range
		...
		for s in S_range
			...
			function eq_c_numeric2!(du, u, p, t)
				ρc = u[1]
				du[1] = eq_c2(ρc, R_value, S_value, T_value, P_value, 0.001)
			end
			...
			# Solve the ODE
			prob = ODEProblem(eq_c_numeric2!, u0, tspan)
			solution2 = ode_solve(prob, Tsit5())
			println("$T_value $S_value $(solution2[end][1])")

			selected_solutions_matrix2[T_index,S_index] = solution2[end][1]
			S_index += 1
		end
		T_index += 1	
	end
...
# ╔═╡ de0a57e3-8247-4629-bc82-bf06af4550ef
md"""
## Phase space
"""

# ╔═╡ a26bf34a-6252-48a6-af64-db6d3b95b482
begin
		# Plot the heatmap of the selected solutions
		heatmap_plot2 = heatmap(T_range, S_range, selected_solutions_matrix2',
        #color=:rainbow,
        clims=(0,1),
        xlabel="T values", ylabel="S values",
        title="Heatmap of Selected Solutions")
		...
		plot(heatmap_plot2)
		# Add text annotation at a specific position (T, S)
		annotate!(heatmap_plot2, 0.5, 0.5, text("HG", :black, 10))
		annotate!(heatmap_plot2, 1.5, 0.5, text("SD", :black, 10))
		annotate!(heatmap_plot2, 0.5, -0.5, text("SH", :red, 10))
		annotate!(heatmap_plot2, 1.5, -0.5, text("PD", :white, 10))
		plot!(heatmap_plot2, [1, 1], [-1, 1], color=:white, linewidth=2, legend=:none)
		plot!(heatmap_plot2, [0, 2], [0, 0], color=:white, linewidth=2, legend=:none)

		# Display the plot
		#show(heatmap_plot)
end

# ╔═╡ f4e4897d-95df-4b09-9333-d913e33f24e7
begin

	solutions_at_T_fixed = selected_solutions_matrix2[10, :]
	
	# Plot S vs the corresponding solutions
	scatter(S_range, solutions_at_T_fixed, xlabel="S", ylabel="Solution", title=" T = 0.9", lw=2, color =:black, legend=:false)	
	plot!(S_range, solutions_at_T_fixed, xlabel="S", ylabel="Solution", lw=2, color =:black)
end
...
manifest_format = "2.0"
project_hash = "7996b7ae434d13051d69aa07fc460b07c0102b3c"
...
[[deps.Arblib]]
deps = ["FLINT_jll", "LinearAlgebra", "Random", "Serialization", "SpecialFunctions"]
git-tree-sha1 = "5e26c6e7783b93e45b1d4bec10ff18c8145da5d8"
...
git-tree-sha1 = "55f6241766b0d0c0157005c9526d1ecf33a12cdc"
...
git-tree-sha1 = "f98a2c9bde925475d34e6d41b171e4e04d9eb54d"
...
git-tree-sha1 = "c75c91f76a47b255844e53109642c3309e2c7cc4"
...
[[deps.Mods]]
git-tree-sha1 = "4ea2e6d62b515e5e986e6ccf1c92a7f878e918d4"
...
git-tree-sha1 = "19a3555ce5d82798b5ab67bdf8b4444d54a159de"
