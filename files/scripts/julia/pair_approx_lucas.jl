...
# ╔═╡ 4e01ce0a-7689-11ef-1e38-d7b2bfd526a6
using Plots, Printf,HomotopyContinuation, LinearAlgebra, DelimitedFiles, PlutoUI

# ╔═╡ 760ad024-ae40-4f80-b497-542b5f4ba341
PlutoUI.TableOfContents(title="", aside=true)

# ╔═╡ 8e7dc4a9-484b-4923-9d94-13a7054f7b9b
md"""
# Pair approximation
"""

# ╔═╡ 6d52fb00-e33d-4285-a2fb-994f895a4a30
md"""
```math
...
# ╔═╡ b78a5c5f-d9ca-47c3-a28c-979086c3952b
md"""
```math
\begin{split}
...
# ╔═╡ 00be331b-f2f0-48c6-aee3-bafd2da286a4
md"""
```math
\begin{split}
...
# ╔═╡ 3d1cd6ff-5834-48eb-95c6-f63e2047f699
...
# ╔═╡ 160e9354-bdec-4b24-9b5c-644f86c100f6
md"""
## Parameters
"""

# ╔═╡ eb4793d1-5817-4445-b64f-42989b1f3757
begin 
...
# ╔═╡ 61fcb2e7-f746-4532-8378-05d665b00556
md"""
## Definitions
"""

# ╔═╡ 539b8ab1-1675-4ea1-8e9b-6cc6ee76abf8
function W_fermi(ΔP, K)
	return 1.0 / (1.0 + exp(-ΔP / K))
end

# ╔═╡ 743a042c-5220-4210-a39f-9d3fc119a6b2
function payoff_C(nc, R, S)
	return  (nc+1)*R + (k - nc)*S
end

# ╔═╡ 2a06dde7-f686-4726-9208-5f9545b8ad7a
function payoff_D(nc, T, P)
	return  (nc+1)*T + (k - nc)*P
end

# ╔═╡ 7590f7e9-c49b-452e-a75c-3c490648888d
# ╠═╡ disabled = true
#=╠═╡
...
# ╔═╡ cff5ea0b-6285-4804-9656-78af519f710a
md"""
## Master eq.
"""

# ╔═╡ 5954015d-19bb-4f82-a136-9605f11f1f7e
...
# ╔═╡ 1b7764e0-d656-4656-a6fe-664ca505fca8
md"""
## Solution
"""

# ╔═╡ ecdfc55a-3192-4d6f-9aa5-2954c326669b
begin
...
# ╔═╡ 15a755f8-dcc8-40d2-9b07-2ab2695dbe56
final_solutions

# ╔═╡ 1b341b45-f699-42dc-9f57-2f631d90596d
md"""
## Plot
"""

# ╔═╡ 4f66d83a-ce0a-4e8c-9d69-6ad7738444dc
monte_carlo_data = readdlm("gzabo_monte_carlo_data.txt", ' ', Float64);

# ╔═╡ 3e4a701b-704c-422b-88ec-c857cc202e75
mendeli_data = readdlm("PD_algebraic.txt", ' ', Float64);

# ╔═╡ 5e4b0575-6ec7-4aff-9689-371cbda61d01
begin
	T_values = [x[1] for x in final_solutions]
	solution = [x[2] for x in final_solutions]
	
	plot(monte_carlo_data[:,1],monte_carlo_data[:,2], label="Monte Carlo",linecolor=:black, markersymbol=:circle)
	scatter!(monte_carlo_data[:,1],monte_carlo_data[:,2], label = "", markercolor=:black, markersymbol=:circle,markersize=2,xlims=(1.1,1.90))
	scatter!(mendeli_data[:,1],mendeli_data[:,2], label = "Mendeli data", markercolor=:red, markersymbol=:circle,markersize=3,xlims=(1.1,1.90))
	scatter!(T_values,solution,xlims=(1.1,3),ylims=(0,1.18), label="pair approx.", markersymbol=:circle, markersize=4, markercolor=:blue, alpha = 0.5, xlabel = "T", ylabel ="ρc")
	#scatter!(results[:,1],results[:,2],markercolor = :blue)
end

# ╔═╡ 02ecf7e6-c7a3-43f4-ac48-a144ee6bfd3a
dados_lucas = readdlm("dados_lucas_symbiotic.txt", ' ',Float64)

# ╔═╡ 0b9878fd-a1fe-4bdf-bf83-363f850995fd
begin
	scatter(dados_lucas[:,1],dados_lucas[:,2], yticks=0.0:0.1:1.0, markersize=2,label="Dados Lucas FPGG")
end
...
manifest_format = "2.0"
project_hash = "c71d9323012adcc72eb3a5cef00f6a2151b77378"
