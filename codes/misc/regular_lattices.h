...
***                      random lattice                           ***
********************************************************************/
void random_lattice (int **viz, int L)
{
	int x,y,z,i,j;
	int N  = L;
	int flag = 0;
		...
		{	
			while(flag!=G*G+G) 
			{
				for(y=1;y<G;y++)
				{
					...
					//int q = (int) N*gsl_rng_uniform(rand_vec);
					
					if(p!=q && connections_number[x]<G-1 && connections_number[p]<G-1)
					{
						viz[x][y] = p;
						viz[p][y] = x; // connection is both ways
				...
				flag=0;				// check if all neighbors from x are different from each other
				for(i=0;i<G;i++)	// goes from zero to check if the if condition above was not met
				{
					for(j=1;j<G;j++)
					{			
						if(viz[x][i] != viz[x][j]){flag += 1;}
					}
				}
			}
		}
