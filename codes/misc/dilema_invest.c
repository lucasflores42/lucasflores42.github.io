...
{	
	// investimento do P x estado P (2) é o investimento real
	// pra P investir 1 botar 1/2
	int i;
	for(i=0;i<N;i++)
	{
		...
		int k = 1 + (int) 4*gsl_rng_uniform(rand_vec);
		double kk;
		if(k==1){kk=1.0;}
		if(k==2){kk=2.0;}
		if(k==3){kk=0.5;}
		...
		int kkk = 8 + (int) 3*gsl_rng_uniform(rand_vec);
		investimento[i] = 10;
...
***                         Update rule                           ***
********************************************************************/
void update_rule( int x, int vizinho, int state[N], double Px, double Py ,int topologia[N], double *investimento, int histograma[N], int t)
{
...
void calculo_mcs(double *payoff, int state[N], int **viz, double r, double gama, double delta, int topologia[N], double *investimento, int histograma[N], int t)
{
	...
	{					
		...
		if (investimento[vizinho] != investimento[x])
		{
			...
			update_rule(x,vizinho,state,Px,Py,topologia,investimento,histograma,t);
...
//------------------------------------------------------------------------------------
		...
		calculo_histograma(t,investimento,histograma,state);
  		...
  		if((NC/(N)==10) || (NP/(N)==1) || (NPC/(N)==1) || (ND/(N)==1) )
		{
			do
			{
