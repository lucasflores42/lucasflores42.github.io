...
#define fps 0000.10000000005 // fps^-1
...
***                          Densidades                           ***
********************************************************************/
void calculo_densidades(int state[N], double *investimento, int histograma[N], int **viz, int t, FILE *fp)
{
	int k;
	...
	for(k=0;k<N;k++)
	{
		if(state[k]==1 && viz[k][1]==1 && viz[k][2]==viz[k][3]==viz[k][4]==0){cross1+=1;}
		if(state[k]==1 && viz[k][1]==viz[k][4]==1 && viz[k][2]==viz[k][3]==0){cross2+=1;}
		if(state[k]==1 && viz[k][1]==viz[k][3]==viz[k][4]==1 && viz[k][2]==0){cross3+=1;}
		if(state[k]==1 && viz[k][1]==viz[k][2]==viz[k][3]==viz[k][4]==1){cross4+=1;}
		if(state[k]==1 && viz[k][1]==viz[k][3]==1 && viz[k][2]==viz[k][4]==0){cross5+=1;}
		if(state[k]==1 && viz[k][1]==viz[k][2]==viz[k][3]==viz[k][4]==0){cross6+=1;}

		if(state[k]==0 && viz[k][1]==1 && viz[k][2]==viz[k][3]==viz[k][4]==0){cross7+=1;}
		if(state[k]==0 && viz[k][1]==viz[k][4]==1 && viz[k][2]==viz[k][3]==0){cross8+=1;}
		if(state[k]==0 && viz[k][1]==viz[k][3]==viz[k][4]==1 && viz[k][2]==0){cross9+=1;}
		if(state[k]==0 && viz[k][1]==viz[k][2]==viz[k][3]==viz[k][4]==1){cross10+=1;}
		if(state[k]==0 && viz[k][1]==viz[k][3]==1 && viz[k][2]==viz[k][4]==0){cross11+=1;}
		if(state[k]==0 && viz[k][1]==viz[k][2]==viz[k][3]==viz[k][4]==0){cross12+=1;}
	}
	N_cross = 4*cross1 + 4*cross2 + 4*cross3 + 1*cross4 + 2*cross5 + 1*cross6 + 4*cross7 + 4*cross8 + 4*cross9 + 1*cross10 + 2*cross11 + 1*cross12;
	printf("%d %f %f %f %f %f %f %f %f %f %f %f %f %f\n",t,cross1/N_cross,cross2/N_cross,cross3/N_cross,cross4/N_cross,cross5/N_cross,cross6/N_cross,cross7/N_cross,cross8/N_cross,cross9/N_cross,cross10/N_cross,cross11/N_cross,cross12/N_cross, N_cross);
}
...
//-------------------C.I.-------------------------------------------------------------
	...
	calculo_densidades(state,investimento,histograma,viz,t,fp);
