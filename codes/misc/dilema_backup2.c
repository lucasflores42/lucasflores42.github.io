...
***                            C.I.                               ***
********************************************************************/		
void calculo_ci(int state[N], int topologia[N], int **viz)
{
	int n;
	...
	{					
		...
		if (state[vizinho] != state[x])
		{
			...
			for(y=0;y<topologia[x];y++)
			{
				...
				calculo_payoff(payoff,r,gama,delta,sitio,topologia,state);
				Px += payoff[state[x]];
				//printf("%d %d %d %d %lf \n", x, state[x],sitio, state[sitio],Px);
			}
			...
			{			
				...
				calculo_payoff(payoff,r,gama,delta,vizinho2,topologia,state);
				Py += payoff[state[vizinho]];
...
//------------------------------------------------------------------------------------
	...
	calculo_ci(state,topologia,viz);
