...
The first thing is to intall raspberry OS 64bit lite on SSD (dont forget to enable ssh)
  ...
  ```
  Important: DONT USE BOOKWORM OS, USE BULLEYE. The former does not allow to config the wifi connection 
  via the wpa_supplicant.conf in roots
```

Now on the SSD, create a file named wpa_supplicant.conf with 
